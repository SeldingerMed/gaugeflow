...
RESULTS="$HERE/results_equiv.jsonl"; : > "$RESULTS"

emit () {
  local arm="$1" cfg="$2" seed="$3" extra="${4:-}" out="$HERE/runs_equiv/$1/seed$3"
...
for s in $SEEDS; do
  ...
  emit gaugeflow "$HERE/config_cardiosyntax_equiv.json"     "$s"   # treatment arm = equivariance (named for analyze.py)
  emit negctrl   "$HERE/config_cardiosyntax_equiv.json"     "$s" "gauge_shuffle=true"
done

"$PY" "$HERE/../common/analyze.py" --results "$RESULTS" --gates "$HERE/gates.json" \
    --direction higher --out "$HERE/verdict_equiv.json" || true
echo "verdict -> $HERE/verdict_equiv.json"
